...
function BuildingLike:Init(buildingPart, triggerPart)
	task.defer(function()
		local enable = BuildingLike:Refresh(buildingPart, triggerPart)
		if not enable then
			local building = Building.TriggerOpenUI(buildingPart, "Like")
			EventManager:Listen(EventManager.Define.RefreshOfficalGroup, function()
				BuildingLike:Refresh(buildingPart, triggerPart)
			end)
		end	
	end)
end

function BuildingLike:Refresh(buildingPart, triggerPart)
