...
local NotifyCheckAniamlEquip = {}

function NotifyCheckAniamlEquip:Handle(rootPart)
	local notifyPart = rootPart:WaitForChild("Notify")
	local function refresh()
		NetClient:RequestQueue({
			{ Module = "Aniaml", Action = "GetEquipMax" },
			{ Module = "Aniaml", Action = "GetEquipCount" },
			{ Module = "Aniaml", Action = "GetPackageCount"}
		}, function(result)
	...
	EventManager:Listen(EventManager.Define.RefreshAniaml, function()
		refresh()
	end)
end

return NotifyCheckAniamlEquip
