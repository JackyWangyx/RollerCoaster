...
local shortStringFromBuffer = require(script.Parent.Parent.Utilities.shortStringFromBuffer)
	...
	if options.inputReceiver then
		table.insert(
			...
			options.inputReceiver.InputBegan:Connect(function(input: InputObject, gpe: boolean?)
				...
				if self._inModal or self._inRightClick then
					return
				end
...
function Tree:_updateBranchFrame(frame, branch, index)
	...
	if not processed then
		...
		elseif typeof(value) == "buffer" then
			frame.Value.Text = shortStringFromBuffer(value)
...
function Tree:_promptPrecisionMenu(options: PrecisionMenuOptions): promptKeyValueForm.FormResult?
	...
	local function updateValueInput()
		...
		elseif valueType == "buffer" then
			...
			local frame = script.EditMenuInputTemplates.BufferEditor:Clone()
			...
			wrapper.styleStates.value = styleState
			frame.BufferLabel.Text = shortStringFromBuffer(defaultValues["buffer"])
			frame.Parent = editFrame.Form.Value

			frame.Link.Activated:Connect(function()
				local bufferWidget = BufferWidget.new(self.theme, defaultValues["buffer"])
				...
				if shouldSubmit then
					...
					frame.BufferLabel.Text = shortStringFromBuffer(lastValue)
				end
...
function Tree:_initializeBranchFrame(frame, index: number)
	...
	local function onRightClick()
		...
		self:_setInModal(true, "rightClick")
...
function Tree:_setInModal(inModal: boolean, inRightClick: "rightClick"?)
...
function Tree:_setHovering(index: number?)
	if self._inModal and not self._inRightClick then
		return
	end
	if self._selectingIsDragging then
		return
